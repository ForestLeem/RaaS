...
class Prohibition(object):
    def __init__(self):
        ...
        self.remedy_list = []               # [0 ... *] remedy (duty)
    ...
    def to_dict(self):
        ...
        # remedy
        if len(self.remedy_list) == 0:
            pass
        else:
            tmp_remedy = self.remedy_list[0].to_dict()
            for i in range(1, len(self.remedy_list)):
                tmp_remedy = add_dict(tmp_remedy, self.remedy_list[i].to_dict())
            merge_dict(tmp_remedy, result)
